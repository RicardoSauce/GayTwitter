...
if __name__ == '__main__':
	...
	n = 120000
	min_count = 10
	filename1 = './GayTwitterTweets.csv'
	filename2 = './tweets2.csv'

	data1 = ingest(filename1)
	data1 = postprocess(data1, n)

	data2 = ingest(filename2)
	data2 = postprocess(data2, n)
	x_train, x_test, y_train, y_test, = train_test_split(np.array(data1.head(n).tokens), np.array(data1.head(n).Sentiment), test_size = 0.3)
	...
	tweet_w2v2 = Word2Vec(size = n_dim, min_count=10)
	...
	word_vectors2 = [tweet_w2v2[w] for w in tweet_w2v2.wv.vocab.keys()]
	print(len(word_vectors))
	print(len(word_vectors2))
	...
	tsne_w2v2 = tsne_model.fit_transform(word_vectors2)
	...
	tsne_df2 = pd.DataFrame(tsne_w2v2, columns =['x','y'])
	tsne_df2['words'] = tweet_w2v2.wv.vocab.keys()
	...
	if True:
		...
		print ('building tf-idf matrix for GayTwitter tweets...')
		...
		print ('building tf-idf matrix for normal tweets...')
		vectorizer = TfidfVectorizer(analyzer=lambda x: x, min_df=10)
